...
export const ResetPasswordPage: React.FC<ResetPasswordPageProps> = ({ token, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (token) {
        const result = await authService.recoverPassword(token, password);
        if (result.success && result.user) {
          ...
          setErrorMsg(result.error || 'Failed to reset password. The recovery link may be expired.');
        }
      } else {
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 space-y-5 text-center">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-slate-600 max-w-sm mx-auto">
            Please enter your new password to regain access to the Cathedral CMS.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white py-8 px-6 sm:px-10 rounded-3xl border border-slate-200 shadow-xl space-y-6">
          ...
          {isSuccess ? (
            <div className="py-6 text-center space-y-3">
              ...
              <h3 className="font-bold text-lg text-slate-900">Password Updated!</h3>
              <p className="text-xs text-slate-600">
                Your new password has been saved. Redirecting to the Admin Portal...
              </p>
            </div>
          ) : (
            <>
